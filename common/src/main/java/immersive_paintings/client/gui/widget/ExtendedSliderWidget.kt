...
abstract class ExtendedSliderWidget<T> (
    ...
    override fun renderWidget(context: DrawContext?, mouseX: Int, mouseY: Int, delta: Float) {
        ...
        context?.drawTexture(texture, this.x + (getOpticalValue() * (this.width - 8).toDouble()).toInt(), this.y, 0, 46 + i, 4, 20)
        context?.drawTexture(texture, this.x + (getOpticalValue() * (this.width - 8).toDouble()).toInt() + 4, this.y, 196, 46 + i, 4, 20)
